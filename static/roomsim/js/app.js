...
(function() {
    ...
    var DEFAULT_WALL = 'sheetrock_half_in_16in_oc';
    ...
    function defaultState() {
        return {
            ...
            surfaces: {
                floor: 'wood_on_joists',
                ceiling: DEFAULT_WALL,
                // wall i runs points[i] → points[i+1]; for a box: S, E, N, W
                walls: [DEFAULT_WALL, 'glass_window', DEFAULT_WALL, DEFAULT_WALL]
            },
    ...
    function normalizeRoom() {
        ...
        state.room.points.forEach(function(p) { p.x -= bb.minX;
            p.y -= bb.minY; });
    ...
    function surfaceRow(list, id, labelText, key, getMat, setMat) {
        ...
        sel.value = getMat();
        sel.addEventListener('change', function() { setMat(sel.value);
            update(); });
        sel.addEventListener('focus', function() { selectedWall = key;
            renderCanvas(); });
        sel.addEventListener('blur', function() { selectedWall = null;
            renderCanvas(); });
        row.appendChild(lab);
        row.appendChild(sel);
    ...
    function buildSourcePanel() {
        ...
        state.sources.forEach(function(src, i) {
            ...
            var fx = numField('x m', src.x, 0, 200, 0.1, function(v) { src.x = v;
                update(); });
            var fy = numField('y m', src.y, 0, 200, 0.1, function(v) { src.y = v;
                update(); });
            var fz = numField('z m', src.z, 0.1, 20, 0.1, function(v) { src.z = v;
                update(); });
            var fl = numField('Lw dB', src.Lw, 40, 140, 1, function(v) { src.Lw = v;
                update(); });
            ...
            qSel.value = src.Q;
            qSel.addEventListener('change', function() { src.Q = parseFloat(qSel.value);
                update(); });
    ...
    function buildListenerPanel() {
        ...
        var fx = numField('x m', L.x, 0, 200, 0.1, function(v) { L.x = v;
            update(); });
        var fy = numField('y m', L.y, 0, 200, 0.1, function(v) { L.y = v;
            update(); });
        var fz = numField('z (ear height) m', L.z, 0.1, 20, 0.1, function(v) { L.z = v;
            update(); });
    ...
    function renderCanvas() {
        ...
        if (viewMode === '3d') View3D.render(payload);
        else Editor.render(payload);
    }
    ...
    function forceUpdate() { lastStaticKey = null;
        update(); }
