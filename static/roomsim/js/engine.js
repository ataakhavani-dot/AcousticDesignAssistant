...
(function (global) {
  ...
  // ADA's MATERIALS dictionary, verbatim. Keys are slugs so they survive URL /
  // storage round-trips; `label` is ADA's own display name.
  var MATERIALS = {
    concrete:    { label: 'Concrete (Reflective)',        alpha: [0.01, 0.01, 0.01, 0.02, 0.02, 0.03] },
    brick:       { label: 'Brick Wall (Reflective)',      alpha: [0.03, 0.03, 0.03, 0.04, 0.05, 0.07] },
    glass:       { label: 'Glass (Reflective)',           alpha: [0.35, 0.25, 0.18, 0.12, 0.07, 0.04] },
    wood_floor:  { label: 'Wood Floor (Reflective)',      alpha: [0.15, 0.11, 0.10, 0.07, 0.06, 0.07] },
    drywall:     { label: 'Drywall (Standard)',           alpha: [0.29, 0.10, 0.05, 0.04, 0.07, 0.09] },
    carpet:      { label: 'Heavy Carpet (Absorptive)',    alpha: [0.02, 0.06, 0.14, 0.37, 0.60, 0.65] },
    foam:        { label: 'Acoustic Foam (Absorptive)',   alpha: [0.08, 0.25, 0.60, 0.90, 0.95, 0.90] },
    fiberglass:  { label: 'Fiberglass 4in (Absorptive)',  alpha: [0.25, 0.90, 1.10, 1.05, 1.00, 1.00] },
    velvet:      { label: 'Velvet Curtains (Absorptive)', alpha: [0.07, 0.31, 0.49, 0.75, 0.70, 0.60] }
  };
  ...
  function centroid(pts) {
    ...
    for (var i = 0; i < pts.length; i++) {
      ...
      a2 += cr; sx += (a.x + b.x) * cr; sy += (a.y + b.y) * cr;
    }
    if (Math.abs(a2) < 1e-9) {
      var bb = bbox(pts);
  ...
  function segsCross(a, b, c, d) {
    var d1 = ccw(a, b, c), d2 = ccw(a, b, d), d3 = ccw(c, d, a), d4 = ccw(c, d, b);
  ...
  function isRectangle(pts) {
    ...
    return { L: Math.hypot(pts[1].x - pts[0].x, pts[1].y - pts[0].y),
  ...
  var geom = {
    signedArea: signedArea, area: area, edges: edges, perimeter: perimeter,
    bbox: bbox, centroid: centroid, pointIn: pointIn, segsCross: segsCross,
    selfIntersects: selfIntersects, isRectangle: isRectangle, ensureCCW: ensureCCW
  };
  ...
  function modes(dims, maxFreq) {
    ...
    for (var n = 1; n < 5; n++) {
      axes.forEach(function (a) {
        ...
        var order = a.n(n);
        out.push({
          f: f, n: n, axis: a.axis, color: AXIS_COLORS[a.axis], type: 'axial',
          nx: order.nx, ny: order.ny, nz: order.nz
        });
      });
    }
  ...
  function computeAll(state) {
    ...
    return {
      abs: abs, rt: rt, R: R,
      tMid: tMid, schroeder: schroeder, target: target,
      rect: rect, modes: modeList, modeFMax: modeFMax,
      ratios: ratios, bolt: bolt,
      splListener: splListener, splOverall: splOverall,
      critical: critical
    };
  }

  global.Acoustics = {
    C: C, BANDS: BANDS, BAND_LABELS: BAND_LABELS, BAND_SHORT: BAND_SHORT,
    MATERIALS: MATERIALS, AXIS_COLORS: AXIS_COLORS, geom: geom,
    surfaceAreas: surfaceAreas, absorption: absorption, rt60: rt60,
    targetRT60: targetRT60, roomRatios: roomRatios, boltArea: boltArea,
    roomConstant: roomConstant, modes: modes, sbirCurve: sbirCurve,
    splAt: splAt, heatmap: heatmap, computeAll: computeAll
  };
